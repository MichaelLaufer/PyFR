...
class NativeReader(object):
    @staticmethod
    def get_native_reader(fname):
        if os.path.isdir(fname):
            return NativeReaderAdios2(fname)
        else:
            return NativeReaderHdf5(fname)

class NativeReaderHdf5(Mapping):
...
class NativeReaderAdios2(Mapping):
    def __init__(self, fname):
        self.fname = os.path.abspath(fname)
        self._file = adios2.open(fname, 'r')
        #self._vars = self._file.available_variables()
        #self._keys = self._vars.keys()
        #self._attrs = self._file.available_attributes()
    ...
    def __contains__(self, aname):
        if isinstance(aname, str):
            ...
            p, q = aname
            return p in self._keys and "{}/{}".format(p,q) in self._attrs

    def __getitem__(self, aname):
        if isinstance(aname, str):
            if self._vars[aname]["Type"] == "string":
                return self._file.read_string(aname)[0]
            else:
                return self._file.read(aname)
        else:
            return self._file.read_attribute_string(aname[1], aname[0])
    ...
    @lazyprop
    def _vars(self):
        return self._file.available_variables()

    @lazyprop
    def _keys(self):
        return self._file.available_variables().keys()
    
    @lazyprop
    def _attrs(self):
        return self._file.available_attributes()
    ...
    def array_info(self, prefix):
        ...
        for i in range(fmaxpn + 1):
            for et in ftypes:
                try:
                    n = f'{prefix}_{et}_p{i}'
                    shape_arr = self._vars[n]['Shape'].split(',')
                    shape_arr = tuple(map(int, shape_arr))
                    info[n] = (et, shape_arr)
                except KeyError:
                    pass
